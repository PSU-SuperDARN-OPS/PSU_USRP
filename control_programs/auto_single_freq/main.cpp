...
#include "../../ionosonde_server/global_variables.h"
...
int main(int argc, char *argv[]){
    ...
    desc.add_options()
        ...
        ("write","write to file")
        ("filename", po::value<std::string>(&autofilename)->default_value("nope"),
            "Filename from auto_single.py")
        ("runcount", po::value<unsigned int>(&runcount)->default_value(0),
            "Run count from auto_single.py")
    ;
    po::variables_map vm;
    po::store(po::parse_command_line(argc, argv, desc), vm);
    po::notify(vm);

//    std::cout << "random string is " << boost::any_cast<std::string>(vm["randomString"].value()) << std::endl;
    std::cout << "passed filename is " << autofilename << std::endl;
    std::cout << "passed run count is " << runcount << std::endl;
    ...
    if (vm.count("write")&&(runcount==1000)){

        /* test filename */
        strftime(fnametest, 80, "ionogram_test.%Y%m%d.%H%M.h5",timeinfo);
        ...
        sprintf(dset, "OPower Test Thing");
        std::cout << dset << std::endl;

        file_id = H5Fcreate(fnametest, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
        std::cout << fnametest << std::endl;
        ...
        if (eval) std::cerr << "Error closing dataspace: " << fnametest << std::endl;

        eval =H5Fclose(file_id);
        if (eval) std::cerr << "Error closing file: " << fnametest << std::endl;
